<script lang="ts">
  ...
  import type { Credit, NewsItem } from '@2min.today/types';
  ...
  const MAX_SPEED = 2.5;
  const MARQUEE_ZONE = 0.25; // trigger within the outer quarter on either side
  ...
  function updateMarqueeSpeed(e: MouseEvent) {
    if (!marqueeEnabled || !rowEl || !rowCanMarquee()) return;
    const rect = rowEl.getBoundingClientRect();
    const relX = (e.clientX - rect.left) / rect.width;
    if (relX > 1 - MARQUEE_ZONE) {
      marqueeSpeed = (relX - (1 - MARQUEE_ZONE)) * 2 * MAX_SPEED; // scroll right
    } else if (relX < MARQUEE_ZONE) {
      marqueeSpeed = (relX - MARQUEE_ZONE) * 2 * MAX_SPEED; // scroll left (negative)
    } else {
      marqueeSpeed = 0;
    }
  }
  ...
  function onRowMouseMove(e: MouseEvent) {
    updateMarqueeSpeed(e);
  }

  function onRowMouseEnter(e: MouseEvent) {
    // startMarquee() resets marqueeSpeed to 0 before scheduling the delayed
    // ease-in. Without a mousemove afterward (e.g. entering straight into the
    // trigger zone from below), that reset value would be what fires 2s later.
    // Set the speed from the entry position itself, after the reset.
    startMarquee();
    updateMarqueeSpeed(e);
  }
